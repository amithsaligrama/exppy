from random import *
guesses = 1
interval_max = int(raw_input("\nWhat is the maximum integer that you want in your interval? "))
interval_min = int(raw_input("What is the minimum integer that you want in your interval? "))
score = interval_max - interval_min
...
while number != user_number:
	if user_number < interval_min or user_number > interval_max:
		...
		print "\nLower\n"
	guesses += 1
	score = int(round(float((interval_max - interval_min) * 10)/guesses))
	user_number = int(raw_input("Try again. "))
print "\n" + str(number) + " is correct!"
print "You took " + str(guesses) + " guesses."
print "Your score is " + str(score) + " out of a possible " + str(10 * (interval_max - interval_min)) + ".\n"
